...
**状态**：**网关侧已完成**（M1R1〜M1R7，2026-09-23）·
退出标准差最后一条，见 §1
...
### 1.1 这条标准现在达成到哪一步（2026-09-23）

| 这一段 | 状态 |
|---|---|
| 提交带首帧图的 I2VA 任务、轮询到完成、下载回 mp4 | ✅ 冒烟脚本一条命令走完，`scripts/smoke_m1.sh` |
| 产物确实是 H.264 MP4 | ✅ 服务器上 ffprobe 断言 |
| **「能在手机上播放」** | ❌ **仍未验过**。App 从 M2R1 起存在了，真机上能连上网关，**但播放要等 M2R5（VS-40）** |

⇒ **M1 不标完成**，它要等 M2 的真机验证把最后一条补上。
⚠️ **不要因为冒烟全绿就把 M1 勾掉。**

M1 **不做**：Android 客户端、媒体库、SD、显存切换、WebSocket 进度推送。
M1 的进度用轮询就够。

⚠️ **2026-09-23 需求扩大之后，「M1 不做」的清单多了两项**：

| 也不做 | 在哪做 |
|---|---|
| 参考图的另外两个来源（从媒体库挑、SD 现生成） | M3 / M5 |
| LoRA 自由挑选 | M6 |

⭐ **这两项都不需要改 M1 已经做完的东西。** 原因是 M1 把「一张参考图」抽象成了
一个 `asset_id` 字符串 —— 三个来源最后都收敛成它，**生成任务的接口一个字不用改**。

⚠️ **但 M1 补了一道守卫**：契约里 `loras` 字段已经存在而实现要等 M6，
中间这个空档会让非空的 `loras` 被 pydantic **静默丢掉** ——
请求通过、任务成功、视频生成，只是没有 LoRA 效果，且哪里都不报错。
现在改成明确拒绝（422）。落在 `models/video_job.py` 的校验器上。
...
一条命令走完：上传图 → 提交 I2VA → 轮询到完成 →
**从网关下载 mp4（验取回链路与 Range）→ 在服务器上 `ffprobe` 断言内容**。

写成 `scripts/smoke_m1.sh`，放进仓库。**以后每次改网关都跑一遍这个。**

⚠️ **原来这里写的是「下载 mp4 → 本地 `ffprobe`」，那条前提是错的** ——
开发机上没有 ffmpeg/ffprobe（runbook §1 早就写了，是这里漏网）。
按 `Docs/Validation.md` §4.1，这属于**前提被推翻**，所以改的是这份文档，
不是在脚本里绕过去。

⚠️ **「下载回来」这一步不能因此省掉**，它和 ffprobe 验的不是同一件事：
**字节在本地验**（取回链路与 Range，VS-11 —— App 就是这么拿视频的），
**像素在服务器验**（编码、尺寸、时长，VS-6）。

本轮还要顺带补三条只有替身证据的判断（VS-19 / VS-20），
以及一次冷启动测量 —— 它们都只有真跑生成才验得了。
...
⚠️ **`H3_TURBO_LORA` 到 M6 也不会进入「用户可挑的 LoRA」那个列表。**
它不是一个「效果」，是 8 步采样 profile 的一部分，由 `turbo` 开关控制。
混进可挑列表里，用户关掉它会得到一个 8 步但没有 Turbo 的垃圾结果 ——
而那不会报错，只会让画质变差。契约 §2 的 LoRA 小节也记了这条。
