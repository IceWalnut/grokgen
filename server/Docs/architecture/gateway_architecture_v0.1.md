...
**状态**：2026-09-23 修订（初稿 2026-09-22）
**上游**：`Docs/requirement/grokgen_requirements_v0.1.md`

⚠️ **2026-09-23 补了两块，都还没实现**：§3.2（LoRA 的底模归属，M6）
与 §3.3（媒体库的图怎么进 `input/`，M3）。
它们跟的是需求文档同日扩大的范围（LoRA 自由挑选、参考图三个来源）。
**两节都只写了「要解决什么问题」和「为什么放在这一层」，没有定做法** ——
做法各有未决的选项，留到对应里程碑开工时定。
...
│   ├── jobs.py          任务状态机 + 串行队列
│   ├── gpu.py           GPU 状态机
│   ├── loras.py         LoRA 目录与底模归属（M6，见 §3.2）
│   └── config.py
├── comfy/
...
├── media/
│   ├── index.py         媒体库索引
│   ├── assets.py        把库里的图引到 input/ 供 LoadImage 用（M3，见 §3.3）
...
### 3.2 LoRA 的底模归属放在 `core/loras.py`（M6）

**问题**：ComfyUI 的 `/object_info` 给的是一个**平铺的文件名列表**，
**不区分底模**。而服务器上 H3 的 4 个和 SD 的 35 个 LoRA 混在同一个目录下
（需求文档 §8.6 有实测）。挑错了要付一次完整的模型加载才会失败。

**为什么放在 `core/` 而不是 `comfy/`**：这不是「和 ComfyUI 说话」的事 ——
ComfyUI 根本不知道这件事。它是网关自己维护的一份知识，
`comfy/client.py` 只负责把那张平铺列表取回来。

**为什么不放在 App 里**：那样每次服务器上新增模型都要发版。
这和 §1.2 决定「不让 App 拼 workflow」是同一条理由。

⚠️ **判断方式还没定**，需求文档 §8.6 列了三条候选（按目录、维护映射表、
读 safetensors 元数据），各有代价。**这个模块的形状取决于选哪条。**

⚠️ 这一层的检查要放在**提交之前**，和现有的 workflow 预检同一层 ——
理由完全相同：提交出去再失败，要先付几分钟的模型加载。

### 3.3 媒体库的图怎么进 `input/`：`media/assets.py`（M3）

**问题**：媒体库的条目在 ComfyUI 的 `output/` 下，
而 workflow 里的 `LoadImage` **读的是 `input/`**。
需求文档 F8 的来源 2（从库里挑一张当首帧）卡在这道坎上。

做法有三条，**M3 开工时定**：软链接、复制、或让 workflow 读绝对路径。

⚠️ **无论选哪条，这一步都必须幂等。** 同一张图被反复拿去生成很正常，
每次都复制一份会让 `input/` 迅速膨胀 —— 而那个目录**已经没有清理机制了**
（上传的图也堆在那儿，见 TODO）。

⭐ **对上层的意义是：三个来源收敛成同一个 `asset_id`。**
上传、从库里挑、SD 现生成，最后都变成一个字符串交给 `POST /v1/jobs` ——
**生成任务的接口一个字都不用改**。这是把它们设计成同一个抽象的好处。
