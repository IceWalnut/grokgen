...
| M1R3 | ComfyUI 客户端 + 第一次真实生成 | VS-5〜7 | 🔄 下一轮 |
| M1R4 | 上传 + 首帧真的生效 | VS-8 | ⬜ |
...
- ⬜ **对齐指令行两种写法哪种对**：Director 文档带 `<>` `[]`，V23 模板示例不带。
  两种 ComfyUI 都不报错，**只影响生成质量**，没有便宜的验证方式。
- ⬜ **turbo LoRA 与 `h3ErosMax_beta5_fp8` 是否兼容**：可能 shape mismatch。
  退路是 `turbo=False`（`res_multistep` / 25 步 / shift 11）。
