...
| 部分 | 状态 |
|---|---|
| §2 任务（提交 / 查询 / 列表 / 取消 / 取回视频） | ✅ **已实现并实跑验证** |
| §2 的 `loras` 字段与 LoRA 小节 | ⚠️ 草案，M6 实现，**未验证** |
| §3 上传 | ✅ 已实现 |
| §4 媒体库 | ⚠️ 草案，M3 |
| §5 GPU 状态 | ⚠️ 草案，M4 |
| §6 事件流 | ⚠️ 草案，**M2 明确不做**（2026-09-23 决定用轮询，见 §6.1） |

**草案部分的形状可能在实现时改。** 写在这里是为了让两端提前知道大致长什么样，
不是承诺。
...
⚠️ **`loras` 是 M6 才实现的字段，现在只接受空数组或不传。**
形状先定在这里，是为了让 App 端不用等到 M6 才知道它长什么样 ——
但**它的形状没有经过任何实现验证**，M6 开工时可能要改。形状定义见下面的
「LoRA」小节。
...
⚠️ **下面这一整节都是草案，没有经过任何实现验证。**
写在这里是为了让 App 端知道形状，M6 开工时可能要改。

`POST /v1/jobs` 的 `loras` 字段：

```json
"loras": [
  { "name": "MysticXXX_MMH3-V4.safetensors", "strength": 0.8 },
  { "name": "riding_pose_H3_i2v_v1.0.safetensors", "strength": 0.6 }
]
```

`name` 是网关列表接口给出的标识，**App 不要自己拼**。

可选的 LoRA 从这里取：

```text
GET /v1/loras?base=h3|sd
```

```json
{ "items": [
    { "name": "MysticXXX_MMH3-V4.safetensors",
      "display_name": "MysticXXX MMH3 V4",
      "base": "h3",
      "trained_words": [],
      "size_bytes": 155095800,
      "source": "manual" }
] }
```

⚠️ **`base` 这个字段是这套接口存在的全部理由。**
ComfyUI 自己的节点定义只给一个平铺的文件名列表，**不区分底模** ——
而服务器上 H3 的 4 个和 SD 的 35 个混在同一个目录下
（需求文档 §8.6 有实测）。让 App 自己按文件名猜是不行的：
`MysticXXX_MMH3-V4` 看得出，`Elieen` 看不出。

⚠️ **`base` 的判断方式还没定**（需求文档 §8.6 列了三条候选，各有代价）。
`source` 字段区分这条记录是怎么来的：`manual`（手工登记）
还是将来下载时自动记的 —— 两种都要能容纳。

⚠️ **Turbo LoRA 不出现在这个列表里。** 它不是一个「效果」，
是 8 步采样 profile 的一部分，由 `turbo` 开关控制。
把它列进来，用户关掉它会得到一个 8 步但没有 Turbo 的垃圾结果。

**挑到不属于当前底模的 LoRA 时，`POST /v1/jobs` 返回 400 并说明原因** ——
不要提交出去等它失败。那要付一次完整的模型加载才知道。
...
### 把库里的图当参考图用（M3，需求文档 F8 来源 2）

```text
POST /v1/library/items/{item_id}/use-as-asset
→ { "asset_id": "grokgen/from_library_itm_xxx.png", "width": 1472, "height": 832 }
```

⭐ **返回的形状和上传接口完全一样**，所以 `POST /v1/jobs` 一个字都不用改 ——
它只认 `asset_id`。三个来源（上传 / 库里挑 / SD 现生成）**在网关这里收敛成同一个东西**。

⚠️ **为什么需要这一步，不能直接把 `item_id` 传给生成任务**：
媒体库的条目在 ComfyUI 的 `output/` 下，而 workflow 里的 `LoadImage`
**读的是 `input/`**。中间这一步就是把图从 output 引到 input
（软链接、复制、还是让 workflow 读绝对路径，M3 开工时定）。

⚠️ **这一步要幂等。** 同一张图被反复拿去生成很正常，
每次都复制一份会让 `input/` 迅速膨胀 —— 而那个目录已经没有清理机制了。

⚠️ **对 App 是一次额外的往返**，但它比重新上传便宜得多：
图本来就在服务器上，这一步不传任何文件内容。
...
### 6.1 2026-09-23 的决定：M2 的 App 用轮询，网关不向 App 推送

**App 每 2 秒调一次 `GET /v1/jobs/{job_id}`，只在前台且任务未到终态时轮询。**
完整理由见 `app/Docs/architecture/android_client_architecture_v0.1.md` §1，
这里只留结论和那条决定性的依据：

推送通常买到两样东西——**减轻服务端被 N 个客户端轮询的压力**、**亚秒级延迟**。
这个项目里客户端数是 1，压力为零；而按 M1R7 的实测配置
（736x416 / 124 帧 / 8 步 Turbo），**采样只有 8 步**，
整个采样阶段能产生的进度更新最多 8 次、平均每 9 秒一次，
**而 2 秒一次的轮询在 75 秒里能问到约 37 次** ——
**推送没有多出来的信息可以给。**

轮询还额外买到一样：每次返回的是**完整当前状态**，漏一次会被下一次自动补上；
推送是增量的，掉一条事件就少一次状态转移。

⭐ **将来真要推送时先看 SSE，不要直接上 WebSocket。**
SSE 单向、走普通 HTTP、FastAPI 原生支持；而 ws 的双向能力这里用不上——
提交与取消都已经是 REST 接口，App 没有需要通过长连接往上发的东西。

⚠️ **「App 不用推送」不等于「网关也不连 ComfyUI 的 ws」。**
§2 里 `stage` 与 `progress` 的数据**只能**来自 ComfyUI 的 WebSocket 事件——
网关现有的两个 ComfyUI 接口（`/queue` 问队列里有哪些任务、`/history/{id}`
问最终结果）**都回答不了「现在跑到第几步」**。
