...
最后更新：2026-09-22（M1R4 完成后）

---

## 1. 现在在哪

**M1R1〜M1R4 已完成。⭐ 从上传一张图到拿到视频，整条链路通了：**
`POST /v1/uploads/image` → `asset_id` → 提交 → ComfyUI 生成 → 首帧就是那张图，
**而且不变形**（画布按图片比例自动推算）。

测试 90 条离线（Level 2）+ 4 条预检（Level 2.5，连 ComfyUI 不占 GPU）
+ 真实生成（Level 3）。

**下一步是 M1R5** —— 任务状态机与串行队列，对应 VS-9、VS-10。
...
显存只够一个重任务，所以网关要有一个容量为 1 的串行队列。
判据：连续提交 3 个任务，**任何时刻只有一个处于 `submitted`/`running`**；
以及失败时 `failure_reason` 里**带着 ComfyUI `node_errors` 的原文**。

⭐ **这一轮纯逻辑 + 替身，不占 GPU。** `FakeComfyClient` 已经能模拟
「拒绝 workflow」和「任务还在排队」两种上游行为。
