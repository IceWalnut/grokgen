...
最后更新：2026-09-22（M1R3 完成后）

---

## 1. 现在在哪

**M1R1〜M1R3 已完成。⭐ 主链路第一次真的跑通了：**
网关拼的 workflow 被 ComfyUI 接受，生成出一个 736x416 / 124 帧 / 5.167 秒的
H.264+AAC MP4，画面正常（红色跑车、海岸、日落）。

测试分三层：66 条离线（Level 2）+ 2 条预检（Level 2.5，连 ComfyUI 不占 GPU）
+ 真实生成（Level 3）。

**下一步是 M1R4** —— 上传 + 首帧真的生效，对应 VS-8。
...
⚠️ 这一条只能靠眼睛：**首帧参数传错时，任务会正常成功，只是悄悄变成一个 T2VA**。
`ffprobe` 能证明文件格式对，证明不了首帧生效了。

⭐ **建议分两步，别一次验两件事**：
ComfyUI 的 `input/` 里已经有 `example.png`，**先用它把 I2VA 链路跑通**
（此时不涉及上传），确认首帧确实出现在画面里；再做 `POST /v1/uploads/image`。
这样「图从哪来」和「首帧生不生效」分开暴露。

⚠️ 提交前先跑一遍预检（`pytest -m integration`）和 `curl /queue`。
