...
⭐ **M1R7 实测证实了「每启动一次进程」这个限定，并量出了另一半**
（4080 SUPER，736x416 / 124 帧 / 8 步 Turbo，各 1 次）：

| 情况 | 一次 5 秒视频的总耗时 |
|---|---|
| 模型已在显存 | 75 秒 |
| `POST /free` 清空显存、**进程还在** | 94 秒（只多付约 19 秒） |
| **进程重启后的第一次** | ❌ 仍未实测 |

⇒ **卸载显存再重载并不贵**（约 19 秒），因为模型还在系统内存里。
贵的是进程重启 —— 那一次要从磁盘读约 29 GB
（主模型 14 GB + text encoder 15 GB），**而这台机器只有 31 GB 内存，
装不下全部**，所以那一次是真的要读盘。

⚠️ 以前这两件事被混成一件。做显存切换设计时（M4）这个区别很关键：
**能用 `/free` 解决就不要重启进程**。
