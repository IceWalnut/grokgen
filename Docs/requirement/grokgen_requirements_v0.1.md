# grokgen v0.1 需求文档

**状态**：2026-09-23 修订（初稿 2026-09-22）
**来源**：`Docs/knowledge/chat_dialogue.json`（2026-09-21～22 与 Codex 在服务器上的对话记录）中用户提出的需求，
加上在两台机器上实测到的环境事实。
**环境事实不在本文档展开**，看 `Docs/runbooks/home_gpu_server.md`。

**2026-09-23 这次改了什么**（用户提出，两项都扩大了 v0.1 的范围）：

1. **LoRA 自由挑选进入 v0.1**（原来在「明确不做」里），视频与生图两边都要。
   **排在最后做** —— 见 F9、§8.6 与里程碑 M6。
2. **参考图从一个来源扩成三个**：手机上传 / 从媒体库挑 / 用 SD 现生成。
   第三个是跨服务的，单独设计，见 F8.1 与里程碑 M5。

⇒ 里程碑相应从 5 个拆成 7 个，**M1–M4 编号未变**。

---

## 1. 一句话目标

做一个 **Android App，当作家里那台 GPU 机器的远程生成控制台**：
在手机上提交 MiniMax H3 视频生成任务和 Stable Diffusion 生图任务，看进度，
浏览和播放机器上已经生成的图片与视频。手机和机器之间走 Tailscale 私有网络。

## 2. 为什么要做

现在的用法是：坐到电脑前，打开浏览器里的 ComfyUI 节点图，手动改参数，排队，等结果，
在 Windows 的文件夹里翻输出。

三个具体不便：

1. **必须在电脑前**。任务动辄十几分钟，人却被绑在机器旁边。
2. **ComfyUI 的节点图不适合手机浏览器**。节点图是为鼠标和大屏做的，手机上点不准、看不全。
3. **输出结果散在服务器的目录里**，没有缩略图、没有分类、手机上看不了。

## 3. 范围

### 3.1 v0.1 做什么

- Android 客户端，通过 Tailscale 访问后端
- 一个**新写的后端网关**，跑在服务器的 WSL 里，是 App 唯一的入口
- MiniMax H3 视频生成：T2VA（纯文本）、I2VA（给首帧图）、FL2VA（给首帧+尾帧）
- Stable Diffusion 生图：文生图，可选图生图
- 任务队列：当前任务、等待队列、取消、重跑、复制参数
- 媒体库：图片网格、视频网格、缩略图、预览、在线播放、下载到手机
- **参考图的三个来源**：手机上传 / 从服务器媒体库里挑 / 先用 SD 生成一张再拿来用。见 F8
- 服务与显存管理：查看当前载入的是哪个模型服务，手动或自动切换
- **LoRA 自由挑选，视频和生图两边都要**。见 F9 与 §8.6

⚠️ **最后两项排在最后做**，它们都依赖前面的东西先立住：
参考图的第三个来源要等 SD 接进来（M4），LoRA 要等两条生成链路都稳定。
排期见 §9。

### 3.2 v0.1 明确不做

⚠️ **「LoRA 自由挑选」原本在这张表里，2026-09-23 移进了 §3.1。**
理由不是它变简单了，而是**它本来就是这个工具的主要用途之一** ——
服务器上已经躺着 39 个 LoRA（4 个 H3 的、35 个 SD 的，见 §8.6），
只用固定那一个 Turbo LoRA 等于把它们全锁起来。
**但它排在最后做**，因为它有一个必须先解决的问题，见 §8.6。

| 不做什么 | 为什么 |
|---|---|
| 手机上的 ComfyUI 节点图编辑器 | 节点图是桌面交互，搬到手机上只会更难用。App 要像一个任务表单，不像一个 IDE |
| REF2VA（多参考图生成） | 当前主模型 `h3ErosMax_beta5_fp8` 更适合 T2V/I2V/FL2VA；真正的 REF2VA 需要另外下载对应模型。详见 §8.3 |
| 多用户、账号体系 | 只有用户一个人用，整个 Tailnet 就是信任边界 |
| 把生成结果自动同步到手机 | 视频文件大，默认在线看，用户点「保存」才下载。详见 F5 |
| 公网访问 | 只走 Tailscale。见 §7 |

## 4. 架构决定：App 不直接连 ComfyUI

```text
Android App
   │  Tailscale 私网（MagicDNS 名，不是 IP）
   ▼
后端网关（新写，跑在 WSL 里）
   ├─→ ComfyUI   127.0.0.1:8188   ← MiniMax H3 与 SD 都走这里（见 §4.2）
   ├─→ 输出目录 / 媒体库索引 / ffmpeg 转码
   └─→ GPU 显存与进程管理
```

### 4.1 网关到底做什么

网关不是「转发一下请求」的薄代理。**App 少不了的那些东西，ComfyUI 一个都不提供**，
这些活必须有人干，而干活的地方只能在服务器上：

| 网关的职责 | 为什么 ComfyUI 干不了 |
|---|---|
| **把表单变成 workflow** —— App 传的是「模式 + prompt + 图 + 时长」，网关把它填进 workflow JSON 再提交 | ComfyUI 的 API 只收完整的节点图。让手机去拼一张几十个节点的 JSON，等于把整个 workflow 的知识搬进 App，换个节点就得发版 |
| **显存与服务切换** —— 维护 GPU 状态机，切换前卸载、轮询显存、必要时重启进程 | ComfyUI 不知道自己该在什么时候让位。这是跨服务的调度，见 F7 |
| **任务队列与去重** —— 保证同一时间只有一个重 GPU 任务 | ComfyUI 的队列只管它自己收到的任务，不管显存够不够 |
| **媒体库索引** —— 扫描输出目录，建缩略图、分页、按来源分类、记录来源任务 | ComfyUI 的 history 只有任务记录，没有可浏览的媒体库 |
| **转码** —— 把 AV1/WebM 转成手机能播的 H.264 MP4（§6） | ComfyUI 的职责到出文件为止 |
| **按 Range 分段传视频** —— 让手机能拖进度条 | ComfyUI 的静态文件服务不是为流媒体设计的 |
| **判断 LoRA 属于哪个底模** —— 把一个平铺的 39 项列表分成「H3 的」和「SD 的」 | ComfyUI 的 `/object_info` 只给文件名，**不带底模信息**，而两个池子混在同一个目录下。这个知识放进 App 就得跟着模型变化发版（见 §8.6） |
| **把媒体库里的图接到生成任务上** —— 产物在 `output/`，而 `LoadImage` 读 `input/` | 这是两个目录之间的事，App 只认 `asset_id`（见 F8.2） |
| **收敛外部入口** —— 只有网关对 Tailscale 暴露，ComfyUI 收回到 `127.0.0.1` | — |

一句话：**网关是 App 和 ComfyUI 之间的翻译层加调度层**。
它存在的最大价值是——以后换 workflow、换模型、换自定义节点，**Android 端不用发版**。

### 4.2 SD 也走 ComfyUI（已定）

**不用 SD WebUI 的 API，在 ComfyUI 里跑 SD workflow。**

理由：只需要管一个进程。SD WebUI 是另一个 venv、另一个端口、另一套 API 和另一份显存占用；
两个进程都常驻，16 GB 显存的切换逻辑要处理「两个都在、都得卸」的组合。
走 ComfyUI 之后，切换退化成「同一个进程里换一套 workflow 和一批模型」，
ComfyUI 自己的模型缓存和 offload 机制就能覆盖大部分情况，网关只在它不够用时才介入重启。

⚠️ 代价要认：**SD 的模型得放进 ComfyUI 的 `models/checkpoints/`**。
**已于 2026-09-22 用目录级软链接解决并验证**，做法与验证见
`server/Docs/implementation/M1_gateway_mainline.md` §5。

`~/stable-diffusion/stable-diffusion-webui/` 保留不动，当作备用与对照，只是不进 v0.1 的链路。

## 5. 功能需求

### F1 连接与服务状态

- App 首次启动时配置后端地址，默认填 MagicDNS 名（形如 `icewalnut-1060.tail22a711.ts.net`），**不要写死 IP**
- 服务页显示：
  - 后端是否可达
  - GPU 型号、总显存、当前已用显存
  - 当前载入的是哪个服务（MiniMax H3 / SD / 空闲）
  - 手动按钮：启动 MiniMax、启动 SD、释放显存、查看日志摘要
- 连不上时要明确区分**四种**情况并分别提示，不要统一显示成「网络错误」：

  | 情况 | 用户要做的事不一样 | 2026-09-24 实测 |
  |---|---|---|
  | **手机没连 Tailscale** | 去 Tailscale App 里连上 | ✅ 分得开 |
  | **服务器离线** | 那台机器关机了，或 WSL 发行版被回收了 | ❌ **和下一条分不开** |
  | **网关进程没起来** | 服务器在，但 7869 上没人监听 | ❌ **和上一条分不开** |
  | **网关活着但 ComfyUI 挂了** | 网关能回话，但它报告上游不可达 | ✅ 分得开 |

  ⚠️ **中间那两条实测证明在当前环境里分不开，这一条要求做不到。**

  判别本来靠「连接被拒绝」和「连接超时」的区别 —— 端口关着而主机活着时，
  正常会收到 RST。但**这台服务器的 WSL 用镜像网络，关闭端口上的连接是被丢弃
  而不是被拒绝**，所以两种情况都只表现为超时。
  杀网关进程、`wsl --shutdown` 各测一次，表现完全一样。

  ⇒ **合并成一档「服务器没有应答」，并把三种可能都写进提示里**，
  让用户知道下一步该去看什么。**挑一种说等于把他指向错误的方向。**

  ⚠️ 这不是把要求放宽，是**一个前提被实测推翻**（`Docs/Validation.md` §4.1）。
  换一台不用镜像网络的机器，这两种就又分得开了。

  ⚠️ **第四种是 2026-09-23 补的，原来只写了三种。**
  起因是查网关实现时发现 `GET /v1/health` **即使 ComfyUI 不可达也返回 200**，
  用响应体里的 `comfy.reachable = false` 告知 —— 而且代码注释写明这是**故意的**：
  做成 502 会让「网关没起来」和「ComfyUI 没起来」在 App 看来一模一样。

  ⇒ **前三种靠传输层异常判断，第四种靠响应体判断。** 两者是不同的判据，
  App 不能只看「请求成不成功」就下结论。

### F2 MiniMax H3 视频生成页

- 模式选择：T2VA（不给图）/ I2VA（一张首帧图）/ FL2VA（首帧 + 尾帧）
  - 实现上 I2VA 和 FL2VA 走同一条路由，区别是给几张图。UI 上仍按三种模式呈现，因为用户心里是三件事
- Prompt 区按现有 Director 的三段结构：
  - `integrated_multimodal_description` —— 画面描述
  - `overall_soundscape` —— 环境声
  - `non_diegetic_music` —— 背景音乐
  - UI 上用三个带说明的输入框，不要直接把这三个英文键名甩给用户
- 首帧图 / 尾帧图槽，**每个槽都能从三个来源取图**（手机上传 / 媒体库 / SD 现生成），见 F8
- 参数：分辨率、时长、steps、seed、比例
- **LoRA 挑选**（可选，可多个，带强度），见 F9
- 高级参数（sampler、scheduler、shift）默认折叠，默认值见 §8.2
- 提交按钮
- 实时状态，至少要能区分这几段，因为它们的耗时量级完全不同：
  排队 → **加载模型**（分钟级）→ 采样 → VAE 解码 → 编码视频

⚠️ **对齐指令行不要让用户手写。** Director 要求给了首/尾帧时，全局 prompt 的第一行必须是一条
描述「第几张图对应第几秒」的对齐句。这由网关或 Director 自动生成。

### F3 Stable Diffusion 生图页

- Prompt / Negative prompt
- 尺寸、steps、CFG、seed、sampler
- 模型选择
- 出图数量
- 图生图：参考图可选，**三个来源与视频页一致**（见 F8）
- **LoRA 挑选**（可选，可多个，带强度），见 F9

⚠️ **SD 生图页有一个视频页没有的职责**：它可能是被视频页「借用」的 ——
用户在视频页点「用 SD 生成首帧」时会进到这里，出图后要能带着那张图回去。
这条路径的设计见 F8。

### F4 队列页

- 当前正在跑的任务及进度
- 等待队列
- 取消任务
- 重跑
- 复制参数到新任务
- 日志摘要（不是整个 ComfyUI 日志，是这个任务相关的那几行）

### F5 媒体库页

- 图片网格 / 视频网格
- 按来源过滤：SD 输出、MiniMax 输出、上传的素材、收藏
- 图片：列表用缩略图 → 点开中等尺寸预览 → 需要时看原图
- 视频：列表用封面帧 → 点开播放
- 下载到手机（显式动作）
- 分享到其他 App

**默认在线看服务器上的文件，不自动下载。** 理由：MiniMax 输出的视频很大，
全量同步到手机不划算；Tailscale 内网带宽够用；缩略图和分页由服务器做，手机只拿它要看的那一份。

后端需要维护索引，至少包含：文件路径、类型、生成时间、来源任务、缩略图路径、时长/尺寸。

### F6 上传（参考图的来源 1）

⚠️ **上传只是三个来源里的一个**，另外两个见 F8。
这一节说的是「图在手机上」那种情况。

- 相册选图、拍照、多图
- 上传前裁剪、压缩、限制尺寸
- **上传与生成分离**：先上传拿到一个 asset id，生成任务里只带 asset id，不带大文件

```json
{ "asset_id": "img_20260922_xxx", "path": "input/mobile/img_20260922_xxx.png" }
```

对 MiniMax H3 来说，首帧图的落地位置就是 ComfyUI 的 `input/` 目录，
再由网关把它塞进 workflow。

### F7 显存与服务切换（**这一项是整个项目最核心的约束**）

GPU 是 RTX 4080 SUPER，**16376 MiB 显存**（实测）。MiniMax H3 这一套里，
主模型 14 GB、text encoder 15 GB —— 单是这两个就已经超过显存总量，
靠 ComfyUI 分批加载和 offload 才跑得起来。**再让 SD 同时常驻是不可能的。**

因此网关必须维护一个显式的 GPU 状态机：

```text
idle
minimax_h3_loaded
sd_loaded
switching_to_minimax
switching_to_sd
busy
```

规则：

1. **同一时间只允许一个重 GPU 任务在跑**，由网关强制，不靠调用方自觉
2. 切换前先停掉当前服务：先尝试 ComfyUI 的 unload，不干净就直接杀进程
3. **轮询显存，等它降到安全阈值再启动另一个**，不要固定 sleep
   （架构文档后来定了用 ComfyUI 的 `/system_stats` 读显存，不用 `nvidia-smi`）
4. App 上显示「正在切换模型服务」并给出预计耗时

⚠️ **对 16 GB 显存这个规模，重启服务往往比复杂的热切换更可靠。**
进程可以都开着，但任务开始前主动 unload；unload 不干净就重启那个进程。

⚠️ **模型加载是一笔与生成量无关的固定开销**，每起一次进程付一次。
所以：**不要为每个任务重启服务**，同一个服务下的连续任务要合并着跑。

### F8 参考图从哪来（三个来源）

**凡是要一张图的地方，都给这三个来源**（视频页的首帧/尾帧、SD 的图生图）：

| 来源 | 说明 | 依赖 |
|---|---|---|
| 1️⃣ **手机上传** | 相册选图、拍照。就是 F6 那条路 | 已有（M1 完成） |
| 2️⃣ **从媒体库挑** | 服务器上已有的图：SD 出过的、以前传过的、收藏的 | 媒体库（M3） |
| 3️⃣ **用 SD 现生成一张** | 当场生图，选一张拿去当首帧 | SD 接入（M4） |

⚠️ **来源 2 比它看起来便宜得多。** 图已经在服务器上了，
选它等于只传一个 id —— **不上传、不下载、不转格式**。
而来源 1 每次都要把图从手机传上去，同一张图用两次就传两次。

⚠️ **来源 3 是这三条里唯一的难点**，下面单独说。

#### F8.1 来源 3：SD 生图 → 作为视频首帧

**这条路径要好好设计，它不是「多加一个按钮」。** 已经能看出来的问题有五个：

**① 它必然包含两次服务切换，而切换是这个项目最贵的操作。**

用户心里是一件事（「生成一个视频」），实际发生的是：

```
切到 SD → 生图 → 切回 H3 → 生视频
```

显存只有 16 GB，两个服务不能同时在（F7）。所以这一次操作要付两次模型切换的代价。
⚠️ **UI 必须把这件事说清楚**，否则用户会以为卡死了 —— 这和 F2 里
「加载模型要单独显示」是同一条理由，只是代价翻倍。

**② 做成一个复合任务，还是两个独立任务？**

**倾向两个独立任务，UI 上把它们串起来。** 理由：

- 任务队列是容量为 1 的串行（F7），复合任务会让状态机多出一堆中间态：
  生图成了但视频失败算什么状态？取消时取消的是哪一个？
- 更重要的是**用户多半想先看看那张图行不行**再决定要不要拿去生成视频。
  强行做成一步，等于剥夺了这个判断点，而生成视频比生图贵得多。

⚠️ 但这个倾向**还没有定**，要在 App 的交互设计里一起定。

**③ 中间那张图算不算媒体库资产？**

**算。** 它就是 SD 的一次正常输出，没有理由特殊对待。
用户可能过几天想再用它生成别的视频 —— 那时它必须在媒体库里找得到。

**④ 宽高比在这里会耦合。**

视频画布是**按首帧图的比例推算**的（避免拉伸变形，见 §8.2 与已实现的行为）。
所以在 SD 页生图时，出图比例就已经决定了后面视频的比例。

⇒ 从视频页跳过去生图时，**应当把「想要的视频比例」带过去**，
作为 SD 出图尺寸的默认值。否则用户出了一张方图，
回来发现视频也变成方的了 —— 而这不会报错。

**⑤ UI 用「选择器」模式，不要用「向导」模式。**

倾向的流程：

```
视频页 → 首帧槽点「用 SD 生成」
  → 进 SD 页（带着回跳标记和比例约束）
  → 生图，可以反复调 prompt 重生成
  → 从结果里选一张「用这张」
  → 回到视频页，首帧槽已填好，其余填过的参数都还在
```

⚠️ **「其余参数都还在」这一条不能丢。** 用户在视频页已经写好了 prompt、
调好了时长，跑去生个图回来发现全清空了，这个功能就没人用了。

#### F8.2 这三个来源对网关意味着什么

对网关来说，**三个来源最终都收敛成同一个东西：一个 `asset_id`**。

- 来源 1：上传接口返回 `asset_id`（已实现）
- 来源 2：媒体库里的条目直接**就是**一个 `asset_id`
- 来源 3：SD 任务的产物是一张图，它同样进媒体库、同样有 `asset_id`

⇒ **生成任务的接口不用改**，它只认 `asset_id`。
三个来源的差别全在 App 侧和媒体库侧，**这是把它们设计成同一个抽象的好处**。

⚠️ 一个要处理的细节：SD 的输出在 ComfyUI 的 `output/` 下，
而首帧图要被 `LoadImage` 读，那个节点读的是 `input/`。
所以来源 2 和 3 需要网关把图**从 output 引到 input**
（做软链接、复制、还是让 workflow 直接读绝对路径，到时候定）。

### F9 LoRA 自由挑选

**视频和生图两边都要。** 服务器上现有 39 个 LoRA（见 §8.6），
现在只有固定那一个 Turbo LoRA 被用上。

要做到：

- 列出可选的 LoRA，**按适用的底模分开**（H3 的归 H3，SD 的归 SD）
- 可以挂多个，每个带一个强度
- 记住上次用过的组合（LoRA 组合是用户反复复用的东西，每次重选很烦）
- 生成参数里带上 LoRA 列表，**并在任务详情里回显** —— 否则事后不知道这个结果是怎么来的

⚠️ **「按适用的底模分开」是这一项真正的难点，不是 UI 问题。**
ComfyUI 只给一个平铺的文件名列表，**不告诉你哪个 LoRA 属于哪个底模**，
而两个池子混在同一个目录下。详见 §8.6。

⚠️ **Turbo LoRA 要和用户挑的 LoRA 区分开。** 它不是一个「效果」，
是采样 profile 的一部分（8 步那套），由 `turbo` 开关控制（见接口契约 §2）。
把它混进用户可挑的列表里，用户关掉它会得到一个 8 步但没有 Turbo 的垃圾结果。

## 6. 播放与传输

- 播放器用 Android 原生 **Media3 / ExoPlayer**
- 后端提供普通 HTTP 视频流，**必须支持 `Range` 请求**，否则进度条拖不动
- 对外统一成 **H.264 + AAC + MP4**，由后端转码

> ⚠️ **本节的结论已被 2026-09-22 的后续实测推翻。**
> 下面「转码必须做」的判断成立于「只能用现有 DaSiWa workflow」这个前提；
> 架构文档定了网关自己拼 workflow 之后，**ComfyUI 原生就能直接输出 H.264 MP4，
> 不需要转码**。见 `server/Docs/architecture/gateway_architecture_v0.1.md` §2.4。
> 本节保留，因为它记录了老输出文件的真实格式——那些文件仍然需要兜底转码。

### 6.1 实测：现有 workflow 的输出不是 MP4

2026-09-22 用 `ffprobe` 查了服务器上两批输出，**两条链路格式完全不同**：

| 来源 | 容器 | 视频编码 | 音频编码 |
|---|---|---|---|
| **DaSiWa workflow**（当前在用）`output/video/2026-09-22/*_audio.webm` | WebM | **AV1** | **Opus** |
| 官方 H3 模板（9 月 19 日跑的）`MiniMax_H3_0000*.mp4` | MP4 | H.264 High | AAC |

⇒ **你现在实际在用的那条链路输出的是 AV1 + Opus 的 WebM。**

AV1 在 Android 上的处境：Android 10 以后要求支持 AV1 解码，但**硬件解码要看具体芯片**，
中低端和老机器多半是软解——软解 AV1 很吃电、容易掉帧。Opus 在 WebM 里兼容性也不如 AAC。
**所以不能指望「手机可能也能播」，转码这一层是必需的，不是优化项。**

### 6.2 转码成本实测：可以忽略

在服务器上把一段 7.3 秒、512×800、3.15 Mbps 的 AV1/WebM 转成 H.264 + AAC 的 MP4：

```bash
ffmpeg -i in.webm -c:v libx264 -preset veryfast -crf 20 -pix_fmt yuv420p \
       -c:a aac -b:a 128k -movflags +faststart out.mp4
```

**实际耗时 0.38 秒**，输出 1.9 MB。相比之下一次视频生成是分钟级的。
⇒ **转码放在生成任务结束后同步做即可**，不需要队列、不需要懒加载、不值得做缓存策略。

两个要点：

1. **`-movflags +faststart` 不能省。** 它把索引挪到文件开头，
   否则手机要先下完整个文件才能起播。
2. **用 CPU 的 `libx264`，不要用 `h264_nvenc`。** 服务器上 NVENC 可用，
   但这台机器的瓶颈是显存而不是 CPU——转码占 0.38 秒 CPU 完全无所谓，
   而去碰 GPU 反而可能和正在跑的生成任务抢资源。

**原始文件保留**，转码产物另存。将来手机端硬解 AV1 普及了，可以直接切回原文件。

### 6.3 接口形状

接口契约**已单独成文**：`Docs/contract/gateway_api_v0.1.md`。
那里是 App 与网关之间唯一的约定，本文档不重复列举 endpoint。

## 7. 非功能需求

| 项 | 要求 |
|---|---|
| 网络 | 只经 Tailscale。**不做公网端口映射、不做内网穿透** |
| 认证 | v0.1 不做账号。Tailnet 成员即可访问。网关**只监听 Tailscale 网卡或 `0.0.0.0`**，不要顺手开到公网 |
| 任务持久性 | 任务跑在服务器上，App 退后台、切网、重启都不影响任务；重连后能看到进度 |
| 失败可见 | 模型切换失败、显存没释放、ComfyUI 进程死掉，都要作为明确状态返回。**不允许用超时或空结果掩盖** |
| 后端存活 | 网关是常驻服务，它活着的前提是 WSL 发行版没被回收 —— 那个坑见 runbook §4 |
| 真机验证 | 播放、上传、相册这三块只能在真机上验证 |
| 视觉 | **App 要好看**，不是"能用就行"。具体要求见 §7.1（2026-09-23 新增） |

### 7.1 视觉与交互要求（2026-09-23 新增）

在这一节之前，本文档里所有关于界面的要求都是**功能性**的
（"显示什么"、"哪个字段不能漏"、"三种失败要分开提示"），
**没有任何一条说界面长什么样**。这一节补上。

#### 7.1.1 方向：深色 + 内容优先

**这个 App 的内容是图和视频，界面是配角。**

| 要求 | 为什么 |
|---|---|
| **只做深色主题，不做浅色** | 内容是生成出来的图和视频，深色背景最衬内容；而且这个 App 大概率是晚上用的。**只有一套配色意味着只需要验一遍** |
| 媒体铺满宽度，不加外框、不加卡片阴影 | 给图加一圈边框等于告诉用户"这是界面的一部分"，而它是结果本身 |
| 强调色**只有一个**，且只出现在主操作与状态上 | 满屏都是强调色等于没有强调色 |
| 输入控件低对比度，退到背景里 | 表单是达成目的的手段，不是目的 |

⚠️ **"只做深色"是一个要认代价的决定**：用户在强光下户外用会比浅色费劲。
判断依据是这个 App 的使用场景 —— 它连的是家里的机器，基本是在家里用。
**将来要加浅色时，token 结构已经在那了，加的是一组值不是一套架构。**

#### 7.1.2 配色（已算过对比度，不是估的）

| 用途 | 色值 | 在 `#0E0F11` 上的对比度 |
|---|---|---|
| 背景 | `#0E0F11` | — |
| 卡面 / 输入区 | `#16181B` | — |
| 次级容器 | `#1F2226` | — |
| 分隔线 | `#2E3339` | 1.51:1（分隔线不承载信息，不要求达标） |
| 正文 | `#E8EAED` | **15.91:1** |
| 次级文字 / 标签 | `#9AA0A6` | **7.26:1** |
| 禁用态 | `#6B7177` | **3.88:1** |
| 强调色 / 运行中 | `#4CC2FF` | **9.56:1** |
| 完成 | `#5BD98A` | **10.72:1** |
| 失败 | `#FF6B6B` | **6.91:1** |
| 排队中 / 已取消 | `#8A9199` | **6.02:1** |
| 主按钮上的文字 | `#06263A` 在 `#4CC2FF` 上 | **7.78:1** |

**对比度是按 WCAG 的相对亮度公式算出来的**，判定线是：
正文 **4.5:1**，UI 组件与大字 **3:1**。上表每一项都达标。

⚠️ **背景用 `#0E0F11` 而不是纯黑 `#000000`**：纯黑在 OLED 屏上滚动时容易拖影，
而且纯黑配纯白文字对比过硬（21:1），长时间看累。

⚠️ **强调色选冷色（天蓝）是有理由的**：生成出来的图和视频色相是任意的，
**冷色强调在暖色内容旁边仍然分得清**，暖色强调（橙/红）会和内容抢。
**它只是一个 token，将来想换换一处即可。**

#### 7.1.3 状态不许只靠颜色区分

任务有五种状态（排队 / 生成中 / 完成 / 失败 / 已取消）。

⚠️ **每一种都必须同时有文字或图标，不能只靠颜色。**
两个理由：色觉障碍；以及**灰色的"排队中"和灰色的"已取消"用的是同一个色值**，
光看颜色本来就分不开。

#### 7.1.4 排版与间距

- **用系统字体，不打包自定义字体。** 界面里有中文，一套覆盖中文的字体文件是 10 MB 级的，
  为一个自用 App 付这个体积不值。
- 间距用 **4dp 的倍数**：4 / 8 / 12 / 16 / 24 / 32。屏幕左右边距 **16dp**。
- 圆角：媒体 **12dp**、卡片 **16dp**、按钮 **12dp**。
  ⚠️ **不用胶囊形按钮** —— 那是 Material You 的标志性形状，会把气质拉回"系统自带工具"。
- 动效克制：状态变化用 **150ms 淡入淡出**，不要弹跳、不要位移。
  ⚠️ 任务状态每 2 秒刷新一次，**动效太重会变成持续晃动**。

#### 7.1.5 这件事放在哪个里程碑

**不设独立的"UI 美化"里程碑。** 拆成两块，因为成本曲线相反：

| 这一块 | 放哪 | 为什么 |
|---|---|---|
| **视觉基础**（配色、字体层级、间距、圆角、组件样式） | **M2R1，和工程骨架一起** | 写一次全 App 继承；不写的话每个页面各自硬编码颜色，到 M4 有七八个页面时再统一要改每一个 Composable。**早做便宜，晚做贵** |
| **逐页的视觉打磨**（布局节奏、空状态、加载态、动效） | 跟着每个页面走 | 成本曲线相反：页面内容还没定就打磨布局是白做的 |

⚠️ **"最后设一个美化里程碑"是最差的选项**：UI 代码写两遍，
而且要在一次改动里同时动七八个页面 —— 而 UI 的验证手段只有真机截图和人眼，
一次改八个页面没法定位是哪一处改坏的。

#### 7.1.6 验收

视觉这件事**机器判不了**，判据落在 `Docs/Validation.md` 的 Level 4（人眼判定）。
但有一条**可以自动检查**：

> **除主题目录外，任何源文件里不许出现颜色字面量。**

这条和分层约束是同一个形式（扫源码、报具体文件与行号），
它守的是"设计能不能被一处改掉" —— 颜色一旦散进各个页面，
换配色就变成一次全仓库搜索替换。

## 8. 已知事实与约束（来自实测和已有部署）

### 8.1 后端跑在哪

ComfyUI 装在服务器 WSL 的 `~/workspace/ComfyUI`，启动脚本和日志在 `~/workspace/minimax_h3`，
启动后监听 `0.0.0.0:8188`（2026-09-22 实测在跑，占用约 13.5 GB 显存）。
起停命令见 runbook §5.1。

SD WebUI 在 `~/stable-diffusion/stable-diffusion-webui`（独立 venv），
**v0.1 不接它** —— SD 走 ComfyUI，理由见 §4.2。

生成结果落在 `~/workspace/ComfyUI/output/video/YYYY-MM-DD/` 下，按日期分目录。
媒体库索引要扫这里。

### 8.2 MiniMax H3 的推荐参数（作为 App 的默认值）

> ⚠️ **本节在 2026-09-22 的 M1R2 更正过。** 原来那组值把两套 profile 混在了一起。

参数分**两套，不能混用**。取自用户在用的 DaSiWa V23 模板里
`Settings & Post-Processing` 那段注释（`Docs/knowledge/` 下的 JSON）：

| | 非 Turbo | **Turbo（用户实际在用，App 默认）** |
|---|---|---|
| Sampler | `res_multistep` | **`euler`** |
| Scheduler | `simple` | `simple` |
| Steps | 25 | **8**（文档给的区间是 4–8） |
| Shift Video | 10–12 | **6**（区间 6–8） |
| Shift Audio | 3–5 | **3**（区间 4–5，模板存的是 3） |

**原来错在哪**：本节最初写的是「sampler 用 `res_multistep`、shift_video 用 11 或 12、
steps 8–12」—— **8 步属于 Turbo 区间，而那两个值是非 Turbo 那一套的**。
这组混合值来自更早一次对着截图的分析，当时没有分 profile 的概念。

**用户实际在用的是 Turbo**：V23 的 Settings 面板存的是
`euler / simple / steps 8 / shift_video 6 / shift_audio 3`。

首次测试的安全分辨率与时长：`736x416`、5 秒（换算后 124 帧 = 5.17 秒）。

⚠️ **`width` / `height` 必须是 32 的倍数**（节点声明了 `step=32`）。
需求早期写过的 `768x432` 无效 —— `432 / 32 = 13.5`。
`736x416` 是服务器上已有成品的真实分辨率。

两个超分开关（`RTX Upscaler & Refiner`、`Latent Upscaler`）**默认都关**，
而且**不要同时开** —— 两个都很吃显存和时间。想要质量就只开 Latent，想要速度就只开 RTX。

`MiniMax H3 Cache` 默认关：它能加速，但可能影响运动连贯性、音频和参考图一致性。

### 8.3 REF2VA 为什么排除在 v0.1 之外

当前主模型是 `h3ErosMax_beta5_fp8.safetensors`，它适合 T2V/I2V/FL2VA。
真正的 reference-to-video 需要对应的 Ref2VA 模型；拿 FL2VA 模型顶替可能报错或结果不对。
要支持 REF2VA，得先下载对应模型 —— 那是后续版本的事。

### 8.4 一个已经踩过的坑要在网关里防住

Video VAE 和 Audio VAE 接反过，报错是 `MiniMax H3 VAE MISMATCH`。
网关构造 workflow 时这两个槽位写死正确的文件名，不要做成可配置项。

### 8.5 客户端环境

开发机上 Android SDK、JDK 17、adb 都已就绪（具体路径见 `AGENTS.md` §4）。
**手机需要装 Tailscale 并登录同一个账号** —— 这是 App 能用的前提。

### 8.6 LoRA 的实际情况（2026-09-23 实测）

服务器上 `~/workspace/ComfyUI/models/loras/` 下现有 **39 个 LoRA**，
分属两个完全不通用的底模：

| 属于谁 | 数量 | 放在哪 |
|---|---|---|
| **MiniMax H3（视频）** | 4 个 | 直接放在 `loras/` 顶层 |
| **Stable Diffusion（生图）** | 31 + 4 个 | 软链接进来的 `sdwebui/`、`sdwebui-lycoris/` 两个子目录 |

H3 那 4 个是：`minimax_h3_fl2v_turbo_8step_v1.0`（就是现在唯一在用的那个）、
`MysticXXX_MMH3-V4`、`riding_pose_H3_i2v_v1.0`、`HMNSFW_AIO_V2`。

#### ⚠️ 难点：ComfyUI 不告诉你哪个 LoRA 属于哪个底模

`/object_info` 给的是一个**平铺的文件名列表**，39 个混在一起，没有任何
「这个是给谁用的」的信息。而两个池子放在同一个目录下，
ComfyUI 的下拉框本来就是这么列的。

**文件名是唯一的线索，而且它不可靠**：

- 看得出来的：`MysticXXX_MMH3-V4`、`riding_pose_H3_i2v_v1.0`、`748cmSDXL`
- **看不出来的**：`HMNSFW_AIO_V2`、`Elieen`、`Amateur_Flash_v2`

⇒ 所以 F9 里「按适用的底模分开」**不能靠解析文件名**。
可行的做法有几条，到时候定：

1. **按目录判断** —— `sdwebui/` 和 `sdwebui-lycoris/` 下面的都是 SD 的，
   顶层的是 H3 的。**现在恰好成立**，但它依赖一个约定而不是事实，
   往顶层丢一个 SD 的 LoRA 就破了。
2. **网关维护一份映射表** —— 最可靠，代价是新增 LoRA 要手工登记一次。
3. **读 safetensors 的元数据头** —— 有些 LoRA 的 header 里记了底模，
   但不是每个都有。**没有验证过，先不当依据。**

⚠️ **挂错了会怎样：大概率报 shape mismatch 而失败** —— 那还算好的，
因为失败是可见的。但要付一次完整的模型加载才知道（见 runbook 的耗时记录），
所以这个检查值得放在提交之前，和现有的 workflow 预检同一层。

## 9. 里程碑与验收

| 里程碑 | 内容 | 验收标准 |
|---|---|---|
| **M1 后端主链路** | 网关能提交一个 MiniMax H3 I2VA 任务、报告状态、取回视频 | 用 `curl` 从开发机提交一次，拿到能播的 mp4 |
| **M2 Android 原型** | 连接、上传图片、填 prompt、提交、播放结果 | 在真机上完整走一遍 I2VA，不碰电脑 |
| **M3 媒体库 + 参考图来源 2** | 扫描输出目录，缩略图，网格，在线播放；**首帧图可以从媒体库里挑**（F8 来源 2） | 手机上能翻到历史输出并播放；**能挑一张库里的图当首帧，全程不重新上传** |
| **M4 接入 SD** | SD 生图 + 服务切换 + 显存释放 | 在 App 里从 MiniMax 切到 SD 再切回来，两次都能出结果，中途显存确实降下去过 |
| **M5 参考图来源 3：SD 生图 → 视频首帧** | F8.1 那条跨服务路径 | 在 App 里一路走完「生图 → 选一张 → 生成视频」，**中途两次服务切换都有明确提示**，回到视频页时先前填的参数还在 |
| **M6 LoRA 自由挑选** | F9，视频与生图两边 | 能挑一个非 Turbo 的 H3 LoRA 生成一次视频、挑一个 SD LoRA 生成一次图；**任务详情里回显用了哪些 LoRA**；挑到不属于当前底模的 LoRA 时**在提交前就被拦下**，不用等一次模型加载 |
| **M7 其余高级功能** | REF2VA、多参考、音频上传、收藏、批量下载 | 逐项定义 |

⚠️ **M5 和 M6 的顺序是有理由的，不要对调。**

M5（SD 生图当首帧）**只是把两条已有的链路接起来** —— M4 之后生图和生视频
都已经能单独跑通，M5 做的是中间那段交互和一次服务切换。

M6（LoRA）**要动两条链路各自的内部**，而且有一个尚未解决的问题
（怎么知道哪个 LoRA 属于哪个底模，见 §8.6）。
把它放最后，是为了在两条链路都已经稳定之后再去改它们。

⚠️ **M1 到 M4 的编号没有变**，M5 之后是新拆的。
原来的 M5「高级功能」被拆成了 M5 / M6 / M7。

## 10. 已定与待定

### 10.1 已定（2026-09-22）

| 问题 | 结论 | 依据 |
|---|---|---|
| SD 走哪条路 | **在 ComfyUI 里跑 SD workflow**，不用 SD WebUI 的 API | §4.2。少管一个进程，显存切换简单得多 |
| 输出是什么编码 | **AV1 + Opus 的 WebM**（当前 DaSiWa 链路），必须转码 | §6.1，`ffprobe` 实测 |
| 转码放哪一步 | 生成结束后同步转成 H.264 + AAC MP4，CPU 编码，原文件保留 | §6.2，实测 0.38 秒 |
| 网关用什么写 | **FastAPI** | 见 §10.2 |
| 网关代码放哪 | **前后端同一个仓库 `IceWalnut/grokgen`（private）**，服务器上 `git pull` 部署 | 见 §10.3 |

### 10.2 网关语言：FastAPI（已定）

**性能不是考虑因素** —— 只有一个用户，两边都远远够用。真正的区别在别处：

**FastAPI（Python）的好处**，也是主要理由：
网关最容易出错的活是**构造 ComfyUI 的 workflow JSON**，尤其是 DaSiWa Director 那套
模式路由、对齐指令行、VAE 槽位的语义。用 Python 就可以直接读、甚至直接 import
ComfyUI 和自定义节点的代码来确认一个字段到底叫什么、取值范围是什么，
也能复用 ComfyUI 社区现成的 API 调用示例。**换成 Go，这部分只能靠人肉对着 JSON 猜。**
另外 ComfyUI 的进度是 WebSocket 事件，Python 这边有现成的客户端写法。

**Go 的好处**：编出来是单个二进制，没有 venv 漂移的问题，常驻更省心；
`net/http` 的 `ServeContent` 自带 Range 支持，不用自己处理分段。

**为什么还是选 FastAPI**：Go 的两个好处都是部署期的便利，而部署只在一台固定机器上做一次；
FastAPI 的好处是开发期的——**它直接降低把 workflow 拼错的概率，而这正是这个项目的主要风险。**
Range 支持用 Starlette 也能做，只是要自己确认一遍，不是障碍。

⚠️ 一个要注意的点：**网关不要和 ComfyUI 共用同一个 venv**。
ComfyUI 的依赖很重且会随更新变动，网关单独建一个 venv，避免互相牵连。

### 10.3 代码放哪、怎么同步（已定）

**前后端放同一个仓库**：`git@github.com:IceWalnut/grokgen.git`（private）。

```text
grokgen/
├── app/                      Android 客户端
├── server/                   FastAPI 网关
├── scripts/deploy_server.sh  部署脚本
└── Docs/
```

**为什么不拆成两个仓库**：App 和网关之间有一份共同的 API 契约，
改一个接口必然要同时改两边。放一个仓库里，**一次改动就是一个 commit**，
不会出现「App 已经发了、网关还没部署」这种对不上的状态。
只有一个人开发、两边一起演进、文档也只有一份——拆开没有任何收益。
将来真要拆（比如网关开源），按目录切出去也不难。

**同步方式：服务器自己 `git pull`。**

```text
开发机（唯一真相源）──push──> GitHub ──pull──> 服务器 ~/workspace/grokgen
```

服务器上那份是 `origin/main` 的**只读检出**，
⚠️ **不要在服务器上改代码** —— 部署脚本用 `git reset --hard origin/main`，改了就没了。
要调试就改开发机、提交、推送、重新部署。

**为什么不直接在服务器上写**，两个理由：

1. **改动要可复审。** 本地文件的每次修改都会渲染成 diff，而且有 git 历史；
   如果源码只在服务器上，每次改动都变成一条 ssh 上的字符串替换命令，
   只有执行的人看得见改了什么。
2. **没有 GPU 也能开发大半。** 网关里只有「调用 ComfyUI」这一小块依赖 GPU。
   把 ComfyUI 客户端放在一个接口后面，其余逻辑（任务状态机、媒体索引、
   Range 分段、转码调用）都能在开发机上跑单元测试。
   ⇒ **这不只是部署方便，它是一条设计约束**：ComfyUI 的调用必须是可替换的，
   否则本地一个测试都跑不了。

部署：`scripts/deploy_server.sh`。它做四件事——
服务器上 `git pull`、装 `server/requirements.txt`、重启 uvicorn、
**确认端口真的在监听**（只看进程起没起来不算验证）。
本地有未 push 的 commit 时脚本会直接拒绝部署，避免部署出一个跟本地不一样的版本。

**venv**：`server/.venv`，由部署脚本创建，**与 ComfyUI 的 venv 分开**。
它在 `.gitignore` 里，不进版本库。

**常驻方式**：服务器上的 WSL 里 systemd 是跑着的（PID 1 是 systemd），
但 `sudo` 需要密码、用户 linger 没开，装不了系统服务。
所以**第一版沿用 ComfyUI 已经在用的那个办法**：`setsid nohup ... &` 脱离 SSH 会话。
想升级成 systemd 服务（自动重启、开机自起）需要一次交互式的 sudo 或
`loginctl enable-linger`，那是后面的事，不挡 M1。

⚠️ 别忘了 runbook §4 那个坑：**网关活着的前提是 WSL 发行版没被回收。**

⚠️ **服务器首次要 bootstrap 一次**：在那台机器上 clone 仓库。
仓库是 private，所以**那台机器需要一把能访问 GitHub 的 SSH key**——
目前还没确认它有没有，见 §10.4。

### 10.4 仍待定

1. **进度怎么推给 App？** 转发 ComfyUI 的 WebSocket 事件，还是让 App 轮询。
   影响 App 端的实时性与耗电。
2. **任务与媒体的元数据存哪？** SQLite 还是 JSON 索引文件。
3. **缩略图什么时候生成？** 生成任务结束时顺手做（和转码同一步），还是首次访问时懒生成。
6. **「SD 生图 → 当首帧」做成一个复合任务还是两个独立任务？**
   倾向两个独立、UI 上串起来（理由见 F8.1 ②），但没定。
   影响任务状态机的形状 —— 所以要在动 M5 之前定，而不是边做边定。
7. **怎么判断一个 LoRA 属于哪个底模？** 三条候选见 §8.6，都各有代价。
   这是 M6 的前置问题。
8. **媒体库里的图怎么给 `LoadImage` 用？** 它们在 `output/` 下，
   而那个节点读 `input/`。软链接、复制、还是让 workflow 读绝对路径（见 F8.2）。
4. ~~SD 的模型怎么归位到 ComfyUI~~ **已完成（2026-09-22）**：目录级软链接，
   已验证 ComfyUI 跟随。见 `server/Docs/implementation/M1_gateway_mainline.md` §5。
5. ~~服务器有没有能访问 GitHub 的 SSH key~~ **已完成**：key 已加，
   服务器上 clone 成功，`git pull` 部署链路实跑通过。
